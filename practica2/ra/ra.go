	...
	// TODO completa
    	...
    	for d := 0; d<ra.NNodes ;d++ { //Liberar a todos los que estaban a la espera de permiso(True)
    		if(ra.RepDefd[d] && d != ra.me-1){ //Si estaba a la espera, hay que liberarlo
    			...
    			ra.RepDefd[d] = false;
    			ra.ms.Send(d+1,ms.Reply{TYPEREPLY,ra.me,linea,escribir})
    		}
    	}
    	ra.Mutex.Unlock()
}
...
func (ra *RASharedDB) Acquire_mutex(){ //Escibir true, leer false
	ra.PreProtocol()
}
...
func (ra *RASharedDB) Listening(){
		for {
			select {
				case<-ra.done:
					ra.Logger.LogLocalEvent("Finishing gorutine listening ra", govec.GetDefaultLogOptions())
					return
				default:
					...
					if(reflect.TypeOf(msg).String() == "ms.Request"){ //Recibido REQUEST
						ra.Mutex.Lock()
							...
							if Defer_it { //Tengo más prioridad asi que pongo al nodo de la petición a esperar
								...
								ra.ms.Send(msg.(ms.Request).Id,ms.Reply{TYPEREPLY,ra.me,"",false})
							}
						...
						ra.Mutex.Lock()
						ra.finished = ra.finished + 1 //Un nodo ha terminado su trabajo
						ra.Mutex.Unlock()
					}
			}
		}
